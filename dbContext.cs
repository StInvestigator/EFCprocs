...
namespace Mirgation06_12
{
    ...
    {
        public string connectionString = @"Data Source=DESKTOP-OF66R01\SQLEXPRESS;Initial Catalog=GamesDB;Integrated Security=True;Connect Timeout=30;Encrypt=False;Trust Server Certificate=False;Application Intent=ReadWrite;Multi Subnet Failover=False";
        ...
        public void Top3StudiosByGames()
        {
            var res = Creators.FromSql($"Top3StudiosByGamesCount").ToList();
            foreach (var item in res)
            {
                Console.WriteLine($"Name: {item.Name}\n");
            }
        }
        public void Top1StudiosByGames()
        {
            var res = Creators.FromSql($"Top3StudiosByGamesCount").ToList();
            Console.WriteLine($"Name: {res.ElementAt(0).Name}\n");
        }
        public void Top3StylesByGames()
        {
            var res = Styles.FromSql($"Top3StylesByGamesCount").ToList();
            foreach (var item in res)
            {
                Console.WriteLine($"Name: {item.Name}\n");
            }
        }
        public void Top1StylesByGames()
        {
            var res = Styles.FromSql($"Top3StylesByGamesCount").ToList();
            Console.WriteLine($"Name: {res.ElementAt(0).Name}\n");
        }
        public void Top3StylesBySoldCopies()
        {
            var res = Styles.FromSql($"Top3StylesBySoldCopies").ToList();
            foreach (var item in res)
            {
                Console.WriteLine($"Name: {item.Name}\n");
            }
        }
        public void Top1StylesBySoldCopies()
        {
            var res = Styles.FromSql($"Top3StylesBySoldCopies").ToList();
            Console.WriteLine($"Name: {res.ElementAt(0).Name}\n");
        }
        public void Top3SinglePlayers()
        {
            var res = Games.FromSql($"Top3SinglePlayers").ToList();
            foreach (var item in res)
            {
                Console.WriteLine($"Name: {item.Name}, Sold Copies: {item.SoldCopies}\n");
            }
        }
        public void Top1SinglePlayer()
        {
            var res = Games.FromSql($"Top3SinglePlayers").ToList();
            Console.WriteLine($"Name: {res.ElementAt(0).Name}, Sold Copies: {res.ElementAt(0).SoldCopies}\n");
        }
        public void Top3MultiPlayers()
        {
            var res = Games.FromSql($"Top3MultiPlayers").ToList();
            foreach (var item in res)
            {
                Console.WriteLine($"Name: {item.Name}, Sold Copies: {item.SoldCopies}\n");
            }
        }
        public void Top1MultiPlayer()
        {
            var res = Games.FromSql($"Top3MultiPlayers").ToList();
            Console.WriteLine($"Name: {res.ElementAt(0).Name}, Sold Copies: {res.ElementAt(0).SoldCopies}\n");
        }
        public void Top1Game()
        {
            var res1 = Games.FromSql($"Top3MultiPlayers").ToList();
            var res2 = Games.FromSql($"Top3SinglePlayers").ToList();
            var res = res1.ElementAt(0).SoldCopies > res2.ElementAt(0).SoldCopies ? res1.ElementAt(0) : res2.ElementAt(0);
            Console.WriteLine($"Name: {res.Name}, Sold Copies: {res.SoldCopies}\n");
        }
        public void DeleteGamesWithLessSoldCopies(int soldCopies)
        {
            var res = Games.FromSql($"DeleteGamesBySales {soldCopies}").ToList();
            Console.WriteLine($"Deleted game(s): {res.Count()}\n");
        }
